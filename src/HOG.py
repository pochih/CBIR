...
def HOG_histogram(input, n_bin=n_bin, type=h_type, n_slice=n_slice, normalize=True):
  ''' count img histogram
...
def _HOG(img, n_bin, normalize=True):
...
def make_sample(db, verbose=True):
  ...
  try:
    ...
    for d in data.itertuples():
      ...
      d_hist = HOG_histogram(d_img, n_bin=n_bin, type=h_type, n_slice=n_slice)
      samples.append({
                      'img':  d_img, 
                      'cls':  d_cls, 
                      'hist': d_hist
