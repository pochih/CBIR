...
class HOG(object):
    ...
    def _HOG(self, img, n_bin, normalize=True):
        ...
        fd = hog(image, orientations=n_orient,
                 pixels_per_cell=p_p_c, cells_per_block=c_p_b)
    ...
    def make_samples(self, db, verbose=True):
        if h_type == 'global':
            sample_cache = "HOG-{}-n_bin{}-n_orient{}-ppc{}-cpb{}".format(
                h_type, n_bin, n_orient, p_p_c, c_p_b)
        elif h_type == 'region':
