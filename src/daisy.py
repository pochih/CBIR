...
def daisy_histogram(input, type=h_type, n_slice=n_slice, normalize=True):
  ''' count img histogram
...
def make_sample(db, verbose=True):
  ...
  try:
    ...
    for d in data.itertuples():
      ...
      d_hist = daisy_histogram(d_img, type=h_type, n_slice=n_slice)
      samples.append({
                      'img':  d_img, 
                      'cls':  d_cls, 
                      'hist': d_hist
