...
def edge_histogram(input, stride=(2, 2), type=h_type, n_slice=n_slice, normalize=True):
  ''' count img histogram
  ...
  if type == 'global':
    hist = _conv(img, stride=stride, kernels=edge_kernels)
    ...
    for hs in range(len(h_silce)-1):
      for ws in range(len(w_slice)-1):
        ...
        hist[hs][ws] = _conv(img_r, stride=stride, kernels=edge_kernels)
...
def _conv(img, stride, kernels, normalize=True):
  H, W, C = img.shape
...
def make_sample(db, verbose=True):
  ...
  try:
    ...
    for d in data.itertuples():
      ...
      d_hist = edge_histogram(d_img, stride=stride, type=h_type, n_slice=n_slice)
      samples.append({
                      'img':  d_img, 
                      'cls':  d_cls, 
                      'hist': d_hist
